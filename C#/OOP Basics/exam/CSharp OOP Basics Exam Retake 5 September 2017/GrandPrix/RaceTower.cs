...
public class RaceTower
{
    ...
    public string CompleteLaps(List<string> commandArgs)
    {
        ...
        if (numberOfLaps > this.track.LapsNumber - this.track.CurrentLap)
        {
            try
            {
                throw new ArgumentException(string.Format(OutputMesages.InvalidLaps, this.track.CurrentLap));
            }
            ...
            for (int index = 0; index < orderedDrivers.Count - 1; index++)
            {
                ...
                if(overtakeHappened)
                {
                    ...
                    builder.AppendLine(string.Format
                        (OutputMesages.OvertakeMessage, overtakingDriver.Name, targetDriver.Name, this.track.CurrentLap));
                }
                else
                {
        ...
        if (this.IsRaceOver)
        {
            ...
            builder.AppendLine(string.Format(OutputMesages.WinnerMessage,winner.Name,winner.TotalTime));
        }
    ...
    public void ChangeWeather(List<string> commandArgs)
    {
        ...
        if (!validWeather)
        {
            throw new ArgumentException(OutputMesages.InvalidWeatherType);
        }
